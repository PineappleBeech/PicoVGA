...
def setup_dma():
    addr = uctypes.addressof(constants)

    machine.mem32[DMA_CH0 + DMA_READ_ADDR] = addr
    machine.mem32[DMA_CH0 + DMA_WRITE_ADDR] = PIO0_TXF0
    machine.mem32[DMA_CH0 + DMA_TRANS_COUNT] = 0xffffffff
    ctrl = 0
    ctrl |= 1 # enable
    ctrl |= 1 << 3 # 4 byte transfers
    ctrl |= DREQ_PIO0_TX0 <<  15 # trigger on PIO0 TX FIFO
